...
|---|---|
| `npm run generate:tests` | Fetch Zephyr cases → AI generates `.spec.ts` files → writes to `src/runner/specs/{platform}/` |
...
- **`generate-tests.yml`** — manual workflow_dispatch; commits generated specs with `[skip ci]`
